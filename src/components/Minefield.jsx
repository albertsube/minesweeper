...
const Minefield = ({
    ...
    if (field[i][j].value === 0) {
      DIRECTIONS.forEach((d) => {
        ...
        if (
          ...
          newX < numRows &&
          newY < numCols &&
          field[newX][newY].isCovered
        )
          uncoverTile(field, newX, newY)
      })
    }
  }
  ...
  const handleClick = (e, i, j) => {
    ...
    if (e.type == 'click' && !newField[i][j].isMarked)
      uncoverTile(newField, i, j)
  ...
  return (
    <div
      ...
      // className={`grid grid-cols-${numCols} gap-3 text-xl`}
